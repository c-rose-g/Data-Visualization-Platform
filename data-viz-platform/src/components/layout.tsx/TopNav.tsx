...
export default function TopNav({sidebarOpen, onHamburgerClick }:HamburgerButtonProp) {
  return (
    <header className="flex items-center h-20 bg-[#0d0d0d] text-white pt-[20px] pb-[20px] pl-[30px] -ml-[80px]">
      <HamburgerButton
      onClick={onHamburgerClick}
      aria-expanded={sidebarOpen}
      className="text-white "
      />
      <nav id="top-nav-btns" className="flex gap-6 items-center pl-[50px] w-full ">
        <a href="/charging" className="bg-[#222324] font-inter text-lg font-medium border border-[#5A5A5A] pt-2 pb-2 pr-6 pl-6 rounded-md">Charging Station</a>
